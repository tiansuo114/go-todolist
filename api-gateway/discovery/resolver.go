...
type Resolver struct {
	...
	cli              *clientv3.Client
	keyPrifix        string
	serverAddersList []resolver.Address
...
func (r *Resolver) update(events []*clientv3.Event) {
	for _, ev := range events {
		...
		case clientv3.EventTypePut:
			...
			if !Exist(r.serverAddersList, addr) {
				r.serverAddersList = append(r.serverAddersList, addr)
				r.cc.UpdateState(resolver.State{Addresses: r.serverAddersList})
			}
		case clientv3.EventTypeDelete:
			...
			if s, ok := Remove(r.serverAddersList, addr); ok {
				r.serverAddersList = s
				r.cc.UpdateState(resolver.State{Addresses: r.serverAddersList})
			}
		}
	}
...
func (r *Resolver) sync() error {
	...
	r.serverAddersList = []resolver.Address{}
